...
export function scoreResource(resource, preferences) {
  let score = 0;
  const reasons = [];
...
export function buildStudyPlan(availableHours, topType) {
...
export function recommendResources(resources, preferences) {
  ...
  const studyPlan = buildStudyPlan(preferences.availableHours, ranked[0]?.type);

  const explanation = ranked.length
    ? `We ranked ${ranked.length} resource${ranked.length === 1 ? '' : 's'} for ${preferences.subject} at ${preferences.educationLevel} level in ${preferences.language}. Subject (+40), education level (+30), language (+20) and matching tags (+10) were scored on this device.`
